...
export function initModels(sequelize: Sequelize) {
  const acquisti = _acquisti.initModel(sequelize);
  const acquisti_commesse = _acquisti_commesse.initModel(sequelize);
  const ana_operatori = _ana_operatori.initModel(sequelize);
  const ana_tipologie = _ana_tipologie.initModel(sequelize);
  const anagrafiche = _anagrafiche.initModel(sequelize);
  const archives = _archives.initModel(sequelize);
  const associazioni_ditte_dogane = _associazioni_ditte_dogane.initModel(sequelize);
  const cdm_associazioni = _cdm_associazioni.initModel(sequelize);
  const cdm_campi = _cdm_campi.initModel(sequelize);
  const cdm_competenze = _cdm_competenze.initModel(sequelize);
  const cdm_letture = _cdm_letture.initModel(sequelize);
  const cdm_marche = _cdm_marche.initModel(sequelize);
  const codici_importo_commessa = _codici_importo_commessa.initModel(sequelize);
  const commesse = _commesse.initModel(sequelize);
  const commesse_associate = _commesse_associate.initModel(sequelize);
  const commesse_sb = _commesse_sb.initModel(sequelize);
  const condomini = _condomini.initModel(sequelize);
  const contract_of_employment = _contract_of_employment.initModel(sequelize);
  const costo_operatori = _costo_operatori.initModel(sequelize);
  const crediti_commesse = _crediti_commesse.initModel(sequelize);
  const credito_debito = _credito_debito.initModel(sequelize);
  const credito_utilizzato = _credito_utilizzato.initModel(sequelize);
  const ditte_dogane = _ditte_dogane.initModel(sequelize);
  const docs_fornitori = _docs_fornitori.initModel(sequelize);
  const dogane = _dogane.initModel(sequelize);
  const dogane_des = _dogane_des.initModel(sequelize);
  const domande = _domande.initModel(sequelize);
  const domande_soddisfazione_cl = _domande_soddisfazione_cl.initModel(sequelize);
  const el_categorie = _el_categorie.initModel(sequelize);
  const el_classificazioni = _el_classificazioni.initModel(sequelize);
  const el_dipendenti = _el_dipendenti.initModel(sequelize);
  const el_personas = _el_personas.initModel(sequelize);
  const el_ruoli = _el_ruoli.initModel(sequelize);
  const el_servizi = _el_servizi.initModel(sequelize);
  const el_settori = _el_settori.initModel(sequelize);
  const el_sottocategorie = _el_sottocategorie.initModel(sequelize);
  const fatt_tipologie = _fatt_tipologie.initModel(sequelize);
  const fatturazione = _fatturazione.initModel(sequelize);
  const fatturazione_commesse = _fatturazione_commesse.initModel(sequelize);
  const fotovoltaico = _fotovoltaico.initModel(sequelize);
  const gruppo_operatori = _gruppo_operatori.initModel(sequelize);
  const gt_timetable = _gt_timetable.initModel(sequelize);
  const hours = _hours.initModel(sequelize);
  const impostazioni_utente = _impostazioni_utente.initModel(sequelize);
  const indicatori = _indicatori.initModel(sequelize);
  const indicatori_obiettivi = _indicatori_obiettivi.initModel(sequelize);
  const installatori = _installatori.initModel(sequelize);
  const ip_bloccati = _ip_bloccati.initModel(sequelize);
  const marche_sim = _marche_sim.initModel(sequelize);
  const metodi_lettura = _metodi_lettura.initModel(sequelize);
  const monitoraggio_oda = _monitoraggio_oda.initModel(sequelize);
  const operatori_commesse = _operatori_commesse.initModel(sequelize);
  const operatori_task = _operatori_task.initModel(sequelize);
  const overtime = _overtime.initModel(sequelize);
  const pagamenti = _pagamenti.initModel(sequelize);
  const pagamenti_task = _pagamenti_task.initModel(sequelize);
  const permessi_operatori = _permessi_operatori.initModel(sequelize);
  const pratiche_enea = _pratiche_enea.initModel(sequelize);
  const preventivi = _preventivi.initModel(sequelize);
  const preventivi_commesse = _preventivi_commesse.initModel(sequelize);
  const provvigioni = _provvigioni.initModel(sequelize);
  const qualifiche = _qualifiche.initModel(sequelize);
  const qualifiche_operatori = _qualifiche_operatori.initModel(sequelize);
  const rinnovo_cpi = _rinnovo_cpi.initModel(sequelize);
  const scia_antincendio = _scia_antincendio.initModel(sequelize);
  const servizi_classificazione = _servizi_classificazione.initModel(sequelize);
  const settore_operatori = _settore_operatori.initModel(sequelize);
  const shifts = _shifts.initModel(sequelize);
  const soddisfazione_cliente = _soddisfazione_cliente.initModel(sequelize);
  const storico_attivita_svolte = _storico_attivita_svolte.initModel(sequelize);
  const storico_ricariche_sim = _storico_ricariche_sim.initModel(sequelize);
  const subtask = _subtask.initModel(sequelize);
  const task_fatturazione = _task_fatturazione.initModel(sequelize);
  const task_programmazione = _task_programmazione.initModel(sequelize);
  const task_subtask = _task_subtask.initModel(sequelize);
  const tasks = _tasks.initModel(sequelize);
  const ticket = _ticket.initModel(sequelize);
  const ticket_commesse = _ticket_commesse.initModel(sequelize);
  const ticket_operatori = _ticket_operatori.initModel(sequelize);
  const timetable = _timetable.initModel(sequelize);
  const tipologia_cl_soddisfazione_cl = _tipologia_cl_soddisfazione_cl.initModel(sequelize);
  const tipologie_credito = _tipologie_credito.initModel(sequelize);
  const todo = _todo.initModel(sequelize);
  const users = _users.initModel(sequelize);
  const valori_qualita = _valori_qualita.initModel(sequelize);
  const valori_stagionali_ditte_dogane = _valori_stagionali_ditte_dogane.initModel(sequelize);
